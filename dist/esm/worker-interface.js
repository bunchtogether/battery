...
export default class BatteryQueueServiceWorkerInterface extends EventEmitter {
  ...
  async updateDurationEstimates(maxDuration = 1000) {
    const port = await this.link();
    await new Promise((resolve, reject) => {
      const requestId = Math.random();
      const timeout = setTimeout(() => {
      ...
      const handleUpdateDurationEstimatesComplete = responseId => {
        if (responseId !== requestId) {
          return;
        }
      ...
      const handleUpdateDurationEstimatesError = (responseId, error) => {
        if (responseId !== requestId) {
          return;
        }
  ...
  async retryQueue(queueId, maxDuration = 1000) {
    const port = await this.link();
    await new Promise((resolve, reject) => {
      const requestId = Math.random();
      const timeout = setTimeout(() => {
      ...
      const handleRetryQueueComplete = responseId => {
        if (responseId !== requestId) {
          return;
        }
      ...
      const handleRetryQueueError = (responseId, error) => {
        if (responseId !== requestId) {
          return;
        }
